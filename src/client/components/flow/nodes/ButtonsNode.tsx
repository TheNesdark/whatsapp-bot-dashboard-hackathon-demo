...
import type { ButtonsNodeData } from '@shared/flow';
import type { FlowNodeProps } from '@/types';

export function ButtonsNode({ data, selected }: FlowNodeProps<ButtonsNodeData>) {
  ...
  return (
    <div
      ref={nodeRef}
      className={`flow-node flow-node--buttons${selected ? ' flow-node--selected' : ''}`}
    >
      ...
      <div className="flow-node__body">
        ...
        {(data.options ?? []).length > 3 && (
          <p className="flow-node__hint">⚠ Máx. 3 botones (WhatsApp)</p>
        )}
      </div>
      {options.map((opt, i) => (
        <Handle
